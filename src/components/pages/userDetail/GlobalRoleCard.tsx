...
export function GlobalRoleCard({
  ...
  return (
    <div className={cn(card.base, "flex flex-col gap-3 p-[22px]")}>
      <h4 className="text-[15.5px] font-bold text-fg-strong">Global role</h4>
      {isSelf ? (
          ...
          {roleChoices.map((choice) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex items-center gap-3 rounded-xl border-[1.5px] px-3.5 py-2.5 text-left transition-colors hover:bg-muted disabled:opacity-60",
              ...
              >
                ...
                <span className="flex flex-col gap-0.5">
                  <span className="inline-flex items-center gap-2 text-[13.5px] font-semibold text-fg-strong">
                    <span className={cn("h-2 w-2 shrink-0 rounded-full", choice.dot)} />
                    {choice.label}
                  </span>
                  <span className="text-[11.5px] text-fg-subtle">{choice.desc}</span>
                </span>
              </button>
            )
          })}
