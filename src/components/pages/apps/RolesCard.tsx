  ...
  return (
    <div className="bg-elevated rounded-[18px] shadow-[var(--shadow-card)] overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-line">
        <h4 className="text-[15.5px] font-semibold text-fg-strong">Roles</h4>
        <span className="text-[11.5px] text-fg-subtle">System roles cannot be deleted (409).</span>
      </div>
      ...
      {loading ? (
        ...
        roles.map((role) => (
          ...
          >
            <span className="text-[13.5px] font-semibold text-fg-strong w-40 truncate">{role.label}</span>
            <span className="font-mono text-xs bg-muted rounded-md px-2 py-0.5 text-fg-muted">{role.role_key}</span>
            {role.is_system && (
              <span className="text-[10.5px] font-bold tracking-[0.08em] uppercase text-fg-subtle bg-muted rounded-full px-2 py-[3px]">
                System
              </span>
            )}
            {!role.is_system && (
              <button
                ...
                className="ml-auto text-[12.5px] font-semibold text-on-accent-soft hover:underline"
              >
                Delete
              </button>
            )}
